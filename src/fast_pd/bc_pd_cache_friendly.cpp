...
//-----------------------------------------------------------------------------//
BC_PD_CF::BC_PD_CF(int R_, int C_, int L_, vector<float> unaries_, vector<float> weights_, vector<float> dist_,
                   vector<float> x_init_)

{
  ...
  std::cout << "MRF with " << N << " nodes (" << R << ", " << C << "), " << L << " labels" << std::endl;
...
void BC_PD_CF::optimize(const size_t I_max, bool grow_sink)
{
  ...
  for(size_t i=0; i<I_max; ++i)
  {
    ...
    std::cout << "NRG new = " << nrg_new << std::endl;
