...
public:
  ...
  void compute_maxflow(const bool grow_sink);
  ...
  //ADD BC
  inline void set_terminal_cap(int v, type_tcap cap_st);
  inline type_tcap get_terminal_cap(int v);
  inline type_ncap get_neighbor_cap(int node_id,int offset_x,int offset_y);
  inline int isSaturated(int node_id) const;
  void reset();
                                                          ...
                                                          const bool grow_sink)
{
  while(!Q_EMPTY())
  {
    ...
    else if (grow_sink & (tree_p==LABEL_T) )
    {
      for(int n=0;n<4;n++)
      {
        const int N = N_ID[n];
...
template <typename type_tcap,typename type_ncap,typename type_flow>
void GridGraph_2D_4C<type_tcap,type_ncap,type_flow>::compute_maxflow(const bool grow_sink)
{
  while(!free_nodes.empty())
  {
    const int v = free_nodes.pop_front();
  ...
  while(true)
  {
    ...
    const bool path_found = grow(vs,vt,st,YOFS, grow_sink);
...
// ADD BC
template <typename type_tcap,typename type_ncap,typename type_flow>
inline void GridGraph_2D_4C<type_tcap,type_ncap,type_flow>::set_terminal_cap(int v, type_tcap cap_st)
{
...
template <typename type_tcap,typename type_ncap,typename type_flow>
inline type_tcap GridGraph_2D_4C<type_tcap,type_ncap,type_flow>::get_terminal_cap(int v)
{
  return RC_ST(v);
}

template<typename type_tcap,typename type_ncap,typename type_flow>
inline type_ncap GridGraph_2D_4C<type_tcap,type_ncap,type_flow>::get_neighbor_cap(int node_id,int offset_x,int offset_y)
{
  static const unsigned char offsets2arc[3][3] = { { 255, EL,255},
    {  LE,255, GE},
    { 255, EG,255} };
  ...
  const int arc = offsets2arc[offset_y+1][offset_x+1];
  return RC(arc, node_id);
}
template

<typename type_tcap,typename type_ncap,typename type_flow>
inline int GridGraph_2D_4C<type_tcap,type_ncap,type_flow>::isSaturated(int v) const
{
  
  return LABEL(v)==LABEL_S && PARENT(v)!=NONE;
}

template <typename type_tcap,typename type_ncap,typename type_flow> void
GridGraph_2D_4C<type_tcap,type_ncap,type_flow>::reset()
{
  size_t mem_pool_size = (W*H*sizeof(unsigned char)+64+         // label
                          W*H*sizeof(unsigned char)+64+         // parent
                          W*H*sizeof(int)+64+                   // parent_id
